...
TL ATTR_RWDATA_IN_TCM uint32_t                                  OctPalsData[16*256];                                    //Table keeping items of all palettes
TL ATTR_RWDATA_IN_TCM uint32_t                                  OctBack[HALFSIDE*HALFSIDE];                             //Back buffer
TL ATTR_RWDATA_IN_TCM uint32_t                                  OctMix[MIX_ROWS*MIX_COLS];                              //Mixing buffer for effects
TL ATTR_RWDATA_IN_TCM uint32_t                                  OctCache[256];                                          //Last unpacked texels (usually it is color indices, not actual colors)
...
//Network
TL octSignalMeet_t                                              OctHwids[CUBES_COUNT] = {0};//OCT_HWID_NONE, OCT_HWID_NONE, OCT_HWID_NONE, OCT_HWID_NONE, OCT_HWID_NONE, OCT_HWID_NONE, OCT_HWID_NONE, OCT_HWID_NONE};
TL uint32_t                                                     OctHwidsNum = 0;
TL ATTR_ZIDATA_IN_NONCACHED_RAM_4BYTE_ALIGN octInfo_t           OctInfo[STREAMS_TOTAL] = {0};                             //The most recent info packets received by this module
TL int                                                          OctNetOutdated = 0;                                     //Critical error. Set when received a packet with seq from far future; forces module to request server to reload; cleared on server reload command
TL uint32_t                                                     OctNetDisconnections = 0;                               //Current maximum number of timed out pings, resets to 0 when everything is connected
TL uint32_t                                                     OctNetDisconnectionStartMs = 0;                         //
TL int                                                          OctDisconnectionPlane = 0;

TL ATTR_ZIDATA_IN_NONCACHED_RAM_4BYTE_ALIGN octStream_t         Streams[STREAMS_TOTAL];                                 //Wrappers around arrays with packets
TL ATTR_ZIDATA_IN_NONCACHED_RAM_4BYTE_ALIGN octPacket_t         CmdPackets[CMDS_CAP];                                   //Circular array storing commands from leader
TL ATTR_ZIDATA_IN_NONCACHED_RAM_4BYTE_ALIGN octPacket_t         BlobPackets[BLOBS_CAP];                                 //Circular array storing blob parts from anyone
TL ATTR_ZIDATA_IN_NONCACHED_RAM_4BYTE_ALIGN octPacket_t         MsgPackets[STREAMS_MSGS * MSGS_CAP];                    //Circular arrays storing messages issued by different peers


TL  uint8_t                                                     OctUartsCache[NET_LINES_MAX][OCT_UART_BUF_CAP];

TL ATTR_ZIDATA_IN_NONCACHED_RAM_4BYTE_ALIGN octJobDownload_t    JobDownload;
...
TL ATTR_ZIDATA_IN_NONCACHED_RAM_4BYTE_ALIGN uint8_t             OctDmaRxBuffers[NET_LINES_MAX][OCT_UART_DMA_CAP];
TL ATTR_ZIDATA_IN_NONCACHED_RAM_4BYTE_ALIGN uint8_t             OctDmaTxBuffers[NET_LINES_MAX][OCT_UART_DMA_CAP];

TL ATTR_RWDATA_IN_TCM volatile octUart_t                        OctUarts[NET_LINES_MAX] = {0};

TL volatile bool                                                OctUartInitialized = false;

TL uint8_t                                                      OctSleepHandleUart = 0;           // Sleep handle to lock sleep while UARTs are active
